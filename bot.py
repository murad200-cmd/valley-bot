...
# 4. عرض الحلقات الخاصة بالجزء المحدد (توليد تلقائي لعدد أكبر من الحلقات)
async def show_episodes(update: Update, context: ContextTypes.DEFAULT_TYPE):
  ...
  keyboard = []
  row = []
  total_episodes = 30  # يمكنك تغيير هذا الرقم لزيادة أو تقليل عدد الحلقات المعروضة لكل جزء
  
  for ep in range(1, total_episodes + 1):
    row.append(
        InlineKeyboardButton(
            f"الحلقة {ep}", callback_data=f"play_{v_type}_{season_num}_{ep}"
        )
    )
    if len(row) == 2:  # زرين في كل سطر لتصبح القائمة منظمة
      keyboard.append(row)
      row = []
  if row:
    keyboard.append(row)

  # زر الرجوع للأجزاء (تم تصحيح اسم الـ callback ليتطابق مع المعالج الصحيح)
...
async def send_episode_file(update: Update, context: ContextTypes.DEFAULT_TYPE):
  ...
  # قاعدة بيانات معرفات الحلقات (مثال: sub_الجزء_الحلقة)
  episodes_db = {
